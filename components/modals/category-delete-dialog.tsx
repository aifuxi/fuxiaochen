...
export const CategoryDeleteDialog = NiceModal.create(({ category, onConfirm }: CategoryDeleteDialogProps) => {
    ...
    <Dialog open={modal.visible} onOpenChange={modal.remove}>
      ...
      `}>
        <DialogHeader>
          ...
          <DialogDescription>
            Remove <span className="text-foreground">{category.name}</span> from the taxonomy. Articles assigned to this
            category will lose the relation after deletion.
          </DialogDescription>
        </DialogHeader>

        <div className="mt-6 rounded-2xl border border-red-400/15 bg-red-400/8 p-4 text-sm leading-6 text-red-100">
          This action cannot be undone. Current usage count: {category.usageCount}.
        </div>
