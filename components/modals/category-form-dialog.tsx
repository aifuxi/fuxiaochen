...
export const CategoryFormDialog = NiceModal.create(({ category, mode, onSubmit }: CategoryFormDialogProps) => {
  ...
  return (
    <Dialog open={modal.visible} onOpenChange={modal.remove}>
      ...
      `}>
        <DialogHeader>
          <DialogTitle className="text-3xl">{mode === "create" ? "Add Category" : "Edit Category"}</DialogTitle>
          <DialogDescription>
            {mode === "create"
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          `}>
            <Field description="Hex color used for category accents." label="Color">
              <div className="flex items-center gap-3">
                <div
                  className="size-10 shrink-0 rounded-lg border border-white/10"
                  style={{ backgroundColor: normalizePreviewColor(values.color) }}
                />
                <Input
                  disabled={isSubmitting}
                  onChange={(event) =>
          ...
          <Field description="Optional context for editors and content planning." label="Description">
            <Textarea
              disabled={isSubmitting}
              onChange={(event) =>
                setValues((currentValues) => ({
