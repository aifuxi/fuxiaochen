      ...
      `}>
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white">
            <span className="font-mono-tech text-xs font-bold text-black">AC</span>
          </div>
        ...
        <div className="flex items-center gap-6">
          {secondaryNavItems.map((item) => (
            <Link key={item.href} className={`
              font-mono-tech text-xs text-muted transition-colors duration-300
        ...
        <span className="font-mono-tech text-xs text-white/40">© 2024 Alex Chen. All rights reserved.</span>
      </div>
    </footer>
  );
}
