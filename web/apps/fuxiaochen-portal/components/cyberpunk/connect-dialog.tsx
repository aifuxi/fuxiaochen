  ...
  return (
    <>
      <button
        ...
        className="px-6 py-2 border border-neon-purple text-neon-purple rounded-full hover:bg-neon-purple/20 transition-all duration-300 text-xs font-bold uppercase tracking-widest shadow-[0_0_10px_rgba(123,97,255,0.2)] hover:shadow-[0_0_20px_rgba(123,97,255,0.4)]"
      >
        建立连接 / Connect
      </button>
      ...
      {isOpen &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <div className="relative w-full max-w-md bg-black/90 border border-neon-purple/50 rounded-xl shadow-[0_0_50px_rgba(123,97,255,0.2)] p-8 space-y-8 animate-in fade-in zoom-in duration-300">
              {/* Header */}
              ...
              <div className="grid grid-cols-3 gap-4">
                {socialLinks.map((link) => (
                  <a
                    ...
                    target="_blank"
                    className={cn(
                      "flex flex-col items-center gap-3 p-4 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-300 group",
                      link.color,
                    )}
                  >
                    <div className="w-10 h-10 flex items-center justify-center rounded-full bg-black/50 group-hover:scale-110 transition-transform duration-300">
                      {link.icon}
                    </div>
              ...
              <a
                href={`mailto:${EMAIL}`}
                className="flex items-center justify-center gap-2 w-full py-3 bg-neon-purple/10 border border-neon-purple/30 text-neon-purple hover:bg-neon-purple/20 transition-all duration-300 rounded-lg text-sm font-bold tracking-wider uppercase group"
              >
                <svg
                  role="img"
                  viewBox="0 0 24 24"
