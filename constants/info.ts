/** 邮箱 */
export const EMAIL = 'aifuxi.js@gmail.com';

/** 昵称 */
export const NICKNAME = '付小晨';

/** Github 个人页 */
export const GITHUB_PAGE = 'https://github.com/aifuxi';

/** 网站代码 Github 地址 */
export const SOURCE_CODE_GITHUB_PAGE =
  'https://github.com/aifuxi/fuxiaochen.com';

/** 网站代码 Gitee 地址 */
export const SOURCE_CODE_GITEE_PAGE = 'https://gitee.com/aifuxi/fuxiaochen.com';

/** 掘金个人页 */
export const JUEJIN_PAGE = 'https://juejin.cn/user/2647279733052494/posts';

/** B站个人页 */
export const BILIBILI_PAGE = 'https://space.bilibili.com/315542317';

/** 网站名称 */
export const WEBSITE = `${NICKNAME}`;

/** Slogan */
export const SLOGAN = `${NICKNAME}，努力做一个更好的程序员。`;
