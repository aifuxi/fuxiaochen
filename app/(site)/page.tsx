...
function Hero() {
  return (
    <section
      className={`relative flex min-h-[70vh] items-center justify-center overflow-hidden py-20 md:py-32`}
    >
      {/* 动态渐变背景 */}
      <div
      ...
      <div className="relative mx-auto max-w-4xl px-4 text-center">
        {/* 小标签 */}
        <div
          className={`mb-8 inline-flex items-center gap-2 rounded-full border border-border/50 bg-surface/50 px-5 py-2 text-sm text-text-secondary backdrop-blur-sm`}
        >
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          ...
          <Link
            href="/about"
            className={`inline-flex items-center gap-2 rounded-full border border-border bg-surface/50 px-8 py-4 text-base font-medium text-text backdrop-blur-sm transition-all duration-300 hover:border-accent/30 hover:bg-surface active:scale-[0.98]`}
          >
            关于我
          </Link>
        </div>
    ...
    <section className={`relative overflow-hidden py-20 md:py-32`}>
      ...
      <div className="relative space-y-10">
        ...
        <Link href={`/blog/${firstPost.slug}`} className="group block">
          <Card
            className={`relative aspect-[21/9] overflow-hidden border-0 p-0 shadow-xl transition-all duration-500 hover:shadow-2xl md:aspect-[21/10]`}
          >
            {firstPost.cover ? (
              <Image
                src={firstPost.cover}
            ...
            <div className={`absolute right-0 bottom-0 left-0 p-6 md:p-10`}>
              {firstPost.category && (
                <span
                  className={`mb-4 inline-block rounded-full bg-white/20 px-4 py-1.5 text-sm font-medium text-white backdrop-blur-sm`}
                >
                  {firstPost.category.name}
                </span>
              )}
              <h2
                className={`mb-3 line-clamp-2 text-2xl font-bold text-white md:text-3xl lg:text-4xl`}
              >
                {firstPost.title}
              </h2>
              <p
                className={`mb-4 line-clamp-2 max-w-2xl text-sm text-white/80 md:text-base`}
              >
                {firstPost.description}
              </p>
              <div
                className={`flex items-center gap-4 text-xs text-white/60 md:text-sm`}
              >
            ...
            {/* 悬浮箭头 */}
            <div
              className={`absolute top-6 right-6 flex h-12 w-12 items-center justify-center rounded-full bg-white/10 text-white opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100`}
            >
        ...
        <div className={`grid gap-6 sm:grid-cols-2 lg:grid-cols-3`}>
          {restPosts.map((post) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="group">
              <Card
                className={`flex h-full flex-col overflow-hidden border-0 bg-surface/50 p-0 transition-all duration-300 hover:bg-surface hover:shadow-xl`}
              >
                {post.cover ? (
                ...
                <div className={`flex flex-1 flex-col gap-3 p-5 md:p-6`}>
                  {post.category && (
                    <span
                      className={`inline-flex w-fit items-center gap-1.5 rounded-full bg-accent/10 px-3 py-1 text-xs font-medium text-accent`}
                    >
                      {post.category.name}
                    </span>
                  )}
                  <h3
                    className={`line-clamp-2 text-lg font-bold text-text group-hover:text-accent md:text-xl`}
                  >
                    {post.title}
                  </h3>
      ...
      <div className="mx-auto max-w-5xl px-4">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {categories.map((category) => (
            ...
            >
              <div
                className={`flex items-center gap-3 rounded-full border border-border bg-surface px-6 py-3 text-base font-medium text-text transition-all duration-300 group-hover:border-accent/30 group-hover:bg-accent group-hover:text-white`}
              >
                <span>{category.name}</span>
                <span
                  className={`rounded-full bg-text/10 px-2.5 py-0.5 text-sm group-hover:bg-white/20`}
                >
                  {category.blogCount}
                </span>
              </div>
...
function BottomCTA() {
  return (
    <section
      className={`relative mb-20 overflow-hidden rounded-3xl bg-gradient-to-br from-accent/5 via-surface to-info/5 py-20 md:py-28`}
    >
      {/* 装饰 */}
      <div
