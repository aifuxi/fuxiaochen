...
export default function CategoriesPage() {
  ...
  const {
    ...
    run: fetchList,
    loading,
  } = useRequest(() => api.get<Category[]>("/categories"), {
  ...
  const { run: remove } = useRequest((id) => api.delete(`/categories/${id}`), {
    manual: true,
    onSuccess: () => {
          ...
          >
            <DialogHeader>
              <DialogTitle className="font-display text-xl tracking-wider text-neon-cyan">
                {editingId ? "EDIT_CATEGORY" : "CREATE_CATEGORY"}
              </DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form
